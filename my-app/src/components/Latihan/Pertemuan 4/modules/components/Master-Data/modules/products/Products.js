  ...
  const getProduct = () => {
    ...
    axios.request(config)
      .then((response) => {
        ...
        setProduct({ ...product, loading: false, data: response.data.data, message:"" });
        //console.log(product)
      })
      .catch((error) => {
        //console.log(error);
        setProduct({ ...product, loading: false, message: error, data:[] });
      });
  }
    ...
    <div id="product-master">
      <div className="row">
        <div className="col-sm-12 col-lg-8">
          ...
          {(product.message) ? <AlertInfo message={product.message.name+". "+product.message.message} variant="danger" /> : ""}
